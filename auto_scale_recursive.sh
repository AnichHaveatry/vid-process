...
#         - 缩放后 sqrt((W*p/q)*(H*p/q)) 尽量接近 1440
#         - 同时系数本身尽量简单（分母尽量小）
#      评分 = |面积相对误差| * 1000 + COMPLEXITY_PENALTY * q，取评分最小者。
...
MAX_Q=12                 # 缩放系数 p/q 的最大分母：越大越精确，但比例越"复杂"
COMPLEXITY_PENALTY=1     # 复杂度惩罚：分母每 +1，相当于抵扣多少"千分比"的误差
...
choose_scale() {
    ...
    BEGIN {
        ...
        best_p = 1; best_q = 1                  # 候选 0：不缩放
        best_score = (P - T) / T * 1000         # 不缩放时面积的相对误差（千分比）

        for (q = 2; q <= maxq; q++) {           # p/q 必须是小整数的简单比
            p = int(q * s + 0.5)
            if (p < 1) p = 1
            if (p >= q) continue                # 必须真的缩小
            err = (p * p * P - q * q * T) / (q * q * T)
            if (err < 0) err = -err
...
while IFS= read -r -d '' f; do
    ...
    if [[ -n ${w:-} && -n ${h:-} ]] && (( w * h > TARGET_AREA )); then
        ...
        if [[ -n ${p:-} && -n ${q:-} ]]; then
            # 与 ffmpeg 的 trunc(iw*p/q/2)*2 等价，保证宽高为偶数
            nw=$(( w * p / q / 2 * 2 ))
            nh=$(( h * p / q / 2 * 2 ))
